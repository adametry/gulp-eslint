...
describe('gulp-eslint failOnProblem', function() {
	...
	describe('gulp-eslint failOnError', function() {
		...
		it('should fail when the file stream ends if an error is found', function(done) {
			...
			lintStream.pipe(eslint.failAfterError().on('error', function(err) {
				should.exists(err);
				err.message.should.equal('Failed with 1 problem');
		...
		it('should fail when the file stream ends if multiple errors are found', function(done) {
			...
			lintStream.pipe(eslint.failAfterError().on('error', function(err) {
				should.exists(err);
				err.message.should.equal('Failed with 2 problems');
				err.name.should.equal('ESLintError');
				err.plugin.should.equal('gulp-eslint');
				done();
			}));
	...
	describe('gulp-eslint failOnWarning', function() {

		it('should fail a file immediately if a warning is found', function(done) {
			var lintStream = eslint({
				envs: ['browser'],
				rules: {
					'no-undef': 1,
			...
			lintStream.pipe(eslint.failOnWarning().on('error', function(err) {
				should.exists(err);
				err.message.should.equal('\"document\" is read only.');
				err.fileName.should.equal('test/fixtures/invalid.js');
				err.plugin.should.equal('gulp-eslint');
		...
		it('should fail when the file stream ends if a warning is found', function(done) {
			var lintStream = eslint({
				envs: ['browser'],
				rules: {
					'no-undef': 1,
			...
			lintStream.pipe(eslint.failAfterWarning().on('error', function(err) {
				should.exists(err);
				err.message.should.equal('Failed with 1 problem');
				err.name.should.equal('ESLintError');
				err.plugin.should.equal('gulp-eslint');
		...
		it('should fail when the file stream ends if multiple warnings are found', function(done) {
			var lintStream = eslint({
				envs: ['browser'],
				rules: {
					'no-undef': 1,
